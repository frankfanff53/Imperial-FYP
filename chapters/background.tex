...
The SLIC algorithm adapted from \cite{achanta2010slic} is illustrated in \autoref{alg:slic}. Note the term "Perturb" in line 2 of \autoref{alg:slic} essentially means to re-assign the cluster center in a \(n \times  n\) neighbourhood, where the re-assigned center is the point has lowest gradient position compare to the original cluster. Additionally, the residual error \(E\) is defined as the \(L_{1}\) distance between previous centers and recomputed centers.
